<template>
	<view class="box">
		<view class="first">
			<image src="https://testfeifanpaopao.jireplayer.com/download/upload/ffpp_xcx/images/play-bg.png" class="bg">
			</image>
			<view class="rank">
				<view class="top">
					<view class="title">运动积分等级排行</view>
					<view class="more" @click="toMore">更多
						<view class="icon">
		...
		<view class="search" id="search">
			<u-input placeholder="请输入赛事关键词" prefixIcon="search" prefixIconStyle="font-size: 22px;color: #909399"
				shape="circle">
		...
		<u-navbar v-if="isSticky">
			<view slot="left">
				<u-search placeholder="请输入赛事关键词" v-model="keyword" bg-color="#F7F7F7" :show-action="false"
					placeholderClass="placeholder-class"></u-search>
			</view>
		</u-navbar>

		<view class="type" :class="{ sticky: isSticky }" :style="{ top: isSticky ? stickyTop : '' }">
			<view class="left">
				<view class="item" @click="selectTab(0)">
					<text :style="{ color: activeTab === 0 ? '#EC384A' : '' }">商圈</text>
					<u-icon name="arrow-down-fill" size="8" :color="activeTab === 0 ? '#EC384A' : '#ccc'"></u-icon>
				</view>
				<view class="item" @click="selectTab(1)">
					<text :style="{ color: activeTab === 1 ? '#EC384A' : '' }">类型</text>
					<u-icon name="arrow-down-fill" size="8" :color="activeTab === 1 ? '#EC384A' : '#ccc'"></u-icon>
				</view>
				<view class="item" style="margin-left: 13px" @click="selectTab(2)">
					<text :style="{ color: activeTab === 2 ? '#EC384A' : '' }">默认排序</text>
					<u-icon name="arrow-down-fill" size="8" :color="activeTab === 2 ? '#EC384A' : '#ccc'"></u-icon>
				</view>
			</view>
			<view class="right">
		...
		<view class="content">
			<view class="item" v-for="(item, index) in matchList" :key="index">
				<view class="left">
					<image :src="item.mainImageUrl" style="width: 90px; height: 90px" mode="aspectFill" />
					<view class="buttom">
				...
				<view class="right">
					...
					<view class="second">海珠区第1名 | {{ item.registerNum }}人 | ￥{{ item.entryFee }}/人
					</view>
					<view class="time">
		...
		<u-popup :show="show" mode="bottom" @close="closeModal" :safeAreaInsetBottom="false">
			<view class="local">
				<view class="top" v-if="activeTab == 0">
					...
					<view class="right">
						<u-radio-group v-model="value" placement="column">
							<u-radio activeColor="red" label="不限"
								customStyle="margin-bottom: 20px;margin-left: 20px"></u-radio>
							<u-radio activeColor="red" label="车陂"
								customStyle="margin-bottom: 20px;margin-left: 20px"></u-radio>
							<u-radio activeColor="red" label="东圃"
								customStyle="margin-bottom: 20px;margin-left: 20px"></u-radio>
						</u-radio-group>
					</view>
				</view>
				<view class="type" v-if="activeTab == 1">
					<view :class="active === index ? 'item-active' : 'item'" v-for="(item, index) in matchLabel"
						:key="index" @click="checkThis(item, index)">{{ item.label_name }}</view>
				</view>
				<view class="sort">
					<u-radio-group v-model="sortType" placement="column">
						<u-radio shape="circle" :label="item.label" :name="item.value" activeColor="red"
							v-for="(item, index) in sortList" :key="index" customStyle="margin-bottom: 20px"></u-radio>
					</u-radio-group>
				</view>
				<view class="next">
