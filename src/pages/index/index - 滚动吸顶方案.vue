	...
	<view>
		<view class="box">
			...
			<view>
				...
				<div class="flow-tabs-top" v-if="bgColor=='#fff'" :style="{top:(44+systemInfo.statusBarHeight)+'px'}">
			...
			<map class="map-example" :zIndex="-1" :latitude="nowAddInfo.latitude" :longitude="nowAddInfo.longitude"
				:markers="markers" :scale="18">
				<!-- 地图可视区域 204px -->
				<view :style="{transform: `translateY(${scrollY}px)`}">
					...
					<!-- 最开始 为 204 -->
					<!-- 最结束 为 830 -->
<!-- @scroll="$u.throttle(pageScrollHandle($event), 5)" -->
					<scroll-view :data-weixin-config="{ inertia: 0.1 }" :scroll-with-animation="true"
						
						:show-scrollbar="false" :bounces="false"
						enhanced enable-back-to-top @scrolltolower="scrolltolowerHandel" :scroll-y="!isDragging"
						:style="{height: scrollViewHeight}" class="list-box">
						<view class="main">
	...
	export default {
		...
		computed: {
			scrollViewHeight() {
				...
				return `calc(100vh - ${this.scrollY+tabsHeight+this.pullBoxHeight-marginToHeight}px)`
			}
		},
		onShow() {
			this.getList();
		...
		methods: {
			...
			handleTouchEnd(e) {
				...
				this.scrollY < (((this.systemInfo.windowHeight / 2) + 30)) ? this.scrollY = 204 : this.scrollY = this
					.systemInfo.windowHeight - 30;
			},
			smoothScrollTo(position) {
				const STEP = 50; // 滚动步长
				const DURATION = 20; // 单步时间
				...
				const timer = setInterval(() => {
					current += delta > 0 ? Math.min(STEP, delta) : Math.max(-STEP, delta);
			...
			pageScrollHandle(e) {
				...
				const topHeight = (this.systemInfo.statusBarHeight + 44 + headTabs + marginToHeight) - (this
					.pullBoxHeight +
					tabsHeight);
				if (this.scrollY <= topHeight) {
					this.scrollY = topHeight;
					this.bgColor = '#fff';
				} else {
					this.scrollY -= parseInt((parseInt(scrollTop)) * 0.05);
					// 备选方案
					// let timmer  = null;
					// timmer = setTimeout(()=>{
					// 	if(this.scrollY == topHeight){
