		...
		<view class="box">
			...
			<map class="map-example" :zIndex="-1" :latitude="nowAddInfo.latitude" :longitude="nowAddInfo.longitude"
				...
				<view :style="{transform: `translateY(${scrollY}px)`}">
					...
					<!-- 最开始 为 204 -->
					<!-- 最结束 为 830 -->
					<!-- @scroll="$u.throttle(pageScrollHandle($event), 100)" -->
					<scroll-view :data-weixin-config="{ inertia: 0.1 }" :scroll-with-animation="true"
						:show-scrollbar="false" :bounces="false" enhanced enable-back-to-top @scroll="scrollHandle"
						@scrolltolower="scrolltolowerHandel" :scroll-y="!isDragging" :style="{height: scrollViewHeight}"
						class="list-box">
						<view class="main">
	...
	export default {
		...
		methods: {
			...
			handleTouchMove(e) {
				...
				console.log(deltaY, 'deltaYdeltaY');
				if (deltaY >= 204 && deltaY <= this.systemInfo.windowHeight) {
					this.scrollY = deltaY;
				}
				if (deltaY <= 204 && deltaY >= 0) {
					this.scrollY = deltaY;
				}
			},
			handleTouchEnd(e) {
				...
				} else if (this.scrollY < ((this.systemInfo.windowHeight / 2) + 30)) {
					this.scrollY = 204;
					this.scrollDoFlag = false
				} else {
					this.scrollY = this.systemInfo.windowHeight - 30;
					this.scrollDoFlag = false
				}
			},
			scrollHandle(e) {
				...
				if(scrollTop==0&&this.scrollDoFlag){
					console.log(111);
				}
			},
