<template>
  <u-sticky :offsetTop="44 + systemInfo.statusBarHeight + 'px'">
    <view class="filter-box">
      <view class="left">
        <view
          @click="controlModalHandle('1')"
          :class="activeSelect == '1' ? 'filter-item active' : 'filter-item'"
        >
          <text>{{
            useAddInfo.street
              ? useAddInfo.street
              : `${useAddInfo.district ? useAddInfo.district : "商圈"}`
          }}</text>
          <view
            :class="
              activeSelect == '1'
                ? 'triangle-down-999C9D active1'
                : 'triangle-down-999C9D'
            "
          ></view>
        </view>
        <view
          @click="controlModalHandle('2')"
          :class="activeSelect == '2' ? 'filter-item active' : 'filter-item'"
        >
        ...
        <view
          @click="controlModalHandle('3')"
          :class="
            activeSelect == '3'
              ? 'filter-item active no-boder'
              : 'filter-item no-boder'
          "
        >
    ...
    <view
      ...
      :style="{
        height: modalShow
          ? `calc(100vh - ${94 + systemInfo.statusBarHeight}px)`
          : 0,
      }"
    >
      <view class="inner" :style="{ height: modalShow ? '664rpx' : 0 }">
        <view class="chose-box" v-if="modalShow">
          ...
          <view class="chose1" v-if="activeSelect == '1'">
            <scroll-view scroll-y="true" class="chose1-left">
              <div
                :class="
                  item.adcode == adInfo.distCode
                    ? 'chose1-left-item active'
                    : 'chose1-left-item'
                "
          ...
          >
            <view class="chose2-box">
              <div
                :class="
                  choseSportTypeInfo.value == item.label_code
                    ? 'chose-2-item active'
                    : 'chose-2-item'
                "
                @click="choseSportTypeHandle(item)"
          ...
          >
            <u-radio-group
              v-model="choseSortTypeInfo.value"
        ...
        <view class="inner-bottom" v-if="modalShow">
          ...
          <view class="confrimBtn" @click="confrimHandle"> 确定 </view>
        </view>
      </view>
      <view
        class="other"
...
export default {
  ...
  methods: {
    ...
    changeHandle(e) {
      console.log(e, "x!xx");
      this.adInfo.street = this.streetList.find(
        (i) => i.street_code == e
      )?.street_name;
    },
    sortChangeHandle(e) {
      console.log(e, "x2xx");
      this.choseSortTypeInfo.name = this.sortTypeList.find(
        (i) => i.value == e
      )?.label;
    },
    ...
    async getSortWayList() {
      const res = await this.$requestAll.dynamics.getDicByName(
        "match_list_sort_type"
      );
