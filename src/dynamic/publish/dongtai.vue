<template>
  <view class="box">
    ...
    <view
      class="zhanWei"
      :style="{ height: 50 + statusBarHeight + 'px' }"
    ></view>
    <view style="margin-top: 24rpx">
      ...
      <view class="siteSelectionBox">
        ...
        <view class="box1">
          ...
          <view
            class="right"
            @click="$utils.toPath.navigateTo('/dynamic/publish/choseMatch')"
          >
    ...
    <view
      class="zhanWei"
      :style="{ height: (keyboardHeight ? 1000 : 0) + 'px' }"
    ></view>
    <!-- 底部按钮 -->
    <view class="bottomBox safe-bottom">
      <view class="layoutBox">
        ...
        <u-button class="publishButton" color="#15181A" @click="submit('1')"
          >发布动态</u-button
        >
      </view>
    </view>
...
export default {
  ...
  methods: {
    async submit(status) {
      ...
      if (this.sportList.length) {
        this.postBody.labelCode = this.sportList
          .map((i) => i.label_code)
          .join(",");
      }
      uni.showLoading({
    ...
    toPreview() {
      ...
      this.$utils.toPath.navigateTo(
        "/dynamic/publish/dongTaiPreview?postBody=" +
          encodeURIComponent(JSON.stringify(postBody))
      );
    },
    toAddLocation() {
  ...
  onShow() {
    ...
    if (uni.getStorageSync("choseAddressInfo")) {
      const choseAddressInfo = JSON.parse(
        uni.getStorageSync("choseAddressInfo")
      );
